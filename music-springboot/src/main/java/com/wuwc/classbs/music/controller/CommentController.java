...
import com.wuwc.classbs.music.service.CommentService;
...
/*添加歌手*/
    ...
    public Object updateComment(HttpServletRequest request){
        JSONObject jsonObject = new JSONObject();
        String id = request.getParameter("id").trim();                   //主
        String up = request.getParameter("up").trim();   //歌单id
        String content = request.getParameter("content").trim();         //评论内容
    ...
    public Object commentOfSongListId(HttpServletRequest request){
        Integer songListId = Integer.parseInt(request.getParameter("songListId"));          //歌曲id
